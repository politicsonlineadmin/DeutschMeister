...
const SYSTEM_PROMPT = `You are DeutschMeister, an expert German language tutor powered by AI. You specialize in teaching German from beginner (A1) to advanced (C2) levels following the CEFR framework.

Core principles:
- Always adapt your language complexity to the learner's current level
- For A1/A2: Use simple English explanations with German examples
- For B1/B2: Mix German and English, gradually increasing German usage
- For C1/C2: Primarily use German with nuanced explanations
...
async function handleGenerateExercise(payload: {
  ...
  const prompt = `Generate a single German language exercise with these parameters:
- CEFR Level: ${level}
- Interest Domain: ${sanitizeString(domain)}
- Exercise Type: ${exercise_type}${grammarClause}

${cefrToNumber(level) >= cefrToNumber('B2') ? 'Include a German-language instruction (instruction_de) since the learner is at B2+.' : ''}
...
Respond with ONLY a JSON object matching this TypeScript interface:
{
  ...
  "instruction_de": string | undefined (German instruction for B2+),
...
async function handleAssessLevel(payload: {
  ...
  const prompt = `Based on the following assessment responses from a German language learner, determine their CEFR level and provide a skill breakdown.
...
async function handleGenerateContent(payload: {
  ...
  "title": string,
  "title_en": string,
  "passage": string (in German, appropriate for ${level}),
  ...
  "items": [{
    ...
    cultural_note: `{
  "title": string,
  "content": string (in English with key German terms),
  "key_phrases": [{ "german": string, "english": string }],
  "did_you_know": string,
  "practical_tip": string
}`,
  };

  const prompt = `Generate ${content_type.replace(/_/g, ' ')} content for a German language learner:
- CEFR Level: ${level}
- Interest Domain: ${sanitizeString(domain)}
